...
class TimelineAgent:
    def __init__(self, llm=None, llm_endpoint=None, omit_tags: List[str] = None, pipeline_variant: str = "full"):
        self.llm = llm
        self.llm_endpoint = llm_endpoint
    ...
    def reconstruct_timeline(self, state: State, n_retries: int = 3) -> State:
        ...
        if self.pipeline_variant == "update_only_final":
            anchor_timeline = state.central_timeline
        else:
            anchor_timeline = state.updated_central_timeline if state.updated_central_timeline else state.central_timeline

        if not anchor_timeline:
            print("Cannot reconstruct timeline because no anchor central timeline is available")
            return state

        # Prepare central timeline string for the prompt
        central_timeline_str = "\n".join(
            f"{event} at {time} hours"
            for event, time in anchor_timeline
        )
...
def main():
    ...
    parser.add_argument('--pipeline_variant',
                       type=str,
                       default='full',
                       choices=['full', 'update_only_central', 'update_only_final'],
    ...
    if args.summary_file:
        ...
        if not should_skip_processing(log_folder, pipeline_variant=args.pipeline_variant):
            run_graph(summary, args.hadm_id, llm, n_retries=args.n_retries, log_folder=log_folder,
                      llm_endpoint=llm_endpoint, omit_tags=omit_tags, pipeline_variant=args.pipeline_variant)
        else:
            print(f"Skipping processing as outputs for variant {args.pipeline_variant} already exist in {log_folder}")
    else:
        ...
        for filename in files:
            ...
            if not should_skip_processing(log_folder, pipeline_variant=args.pipeline_variant):
                ...
                print(f"\nSkipping {filename} as outputs for variant {args.pipeline_variant} already exist in {log_folder}")
...
UPDATE_ONLY_CENTRAL_PIPELINE_STEPS = FULL_PIPELINE_STEPS[:-1]

UPDATE_ONLY_FINAL_PIPELINE_STEPS = [
    {
        "name": "central_events",
        "primary_output": "central_events.json",
        "outputs": ["central_events.json"],
    ...
    },
    {
        ...
        "primary_output": "timeline.json",
        "outputs": ["timeline.json"],
        "dependencies": ["central_timeline.json", "non_central_events.json"],
        "runner": "reconstruct_timeline",
    },
    {
        "name": "updated_timeline",
...
PIPELINE_STEPS_BY_VARIANT = {
    "full": FULL_PIPELINE_STEPS,
    "update_only_central": UPDATE_ONLY_CENTRAL_PIPELINE_STEPS,
    "update_only_final": UPDATE_ONLY_FINAL_PIPELINE_STEPS,
}


def get_pipeline_steps(pipeline_variant: str) -> List[Dict[str, Any]]:
    if pipeline_variant not in PIPELINE_STEPS_BY_VARIANT:
        raise ValueError(f"Unsupported pipeline variant: {pipeline_variant}")
    return PIPELINE_STEPS_BY_VARIANT[pipeline_variant]


def get_expected_output_files(pipeline_variant: str) -> List[str]:
    expected: List[str] = []
    for step in get_pipeline_steps(pipeline_variant):
...
def should_skip_processing(log_folder: str, pipeline_variant: str = "full") -> bool:
    """Skip only when every expected output file for the selected variant is present and non-empty."""
    for filename in get_expected_output_files(pipeline_variant):
...
def run_graph(discharge_summary: str, hadm_id: int, llm: Any, n_retries: int = 3,
              log_folder: str = None, llm_endpoint: str = None, omit_tags: List[str] = None,
              pipeline_variant: str = "full") -> State:
    agent = TimelineAgent(llm=llm, llm_endpoint=llm_endpoint, omit_tags=omit_tags, pipeline_variant=pipeline_variant)
